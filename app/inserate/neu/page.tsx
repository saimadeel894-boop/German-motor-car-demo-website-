...
const focusStyle = (focused) => ({ borderColor: focused ? "#E63027" : "#DDE3EA", boxShadow: focused ? "0 0 0 3px rgba(230,48,39,0.08)" : "none" });
  ...
  return (
    <div>
      ...
      <div onClick={() => !processing && fileRef.current?.click()}
        ...
        style={{ border:"2px dashed #C8D5E0", borderRadius:14, padding:"28px 20px", textAlign:"center", background:"#F7F9FB", cursor: processing ? "wait" : "pointer", marginBottom:16, transition:"all 0.2s ease" }}>
        {processing ? (
          <>
  ...
  <div style={{ textAlign:"center", padding:"40px 24px" }}>
    ...
    {form.images.length > 0 && (
      <div style={{ marginBottom:24, borderRadius:12, overflow:"hidden", maxHeight:160 }}>
        <img src={form.images[0].url} alt="Ihr Fahrzeug" style={{ width:"100%", objectFit:"cover", maxHeight:160 }} />
      </div>
    )}
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      <a href="/" style={{ display:"block", padding:"14px", borderRadius:12, border:"none", background:"#1C3557", color:"#fff", fontSize:15, fontWeight:700, fontFamily:"'DM Sans',sans-serif" }}>
        Mein Inserat ansehen →
      </a>
      <button onClick={() => window.location.reload()} style={{ padding:"14px", borderRadius:12, border:"1.5px solid #DDE3EA", background:"transparent", color:"#1C3557", fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ minHeight:"100vh", background:"#F0F4F8", display:"flex", flexDirection:"column", alignItems:"center", padding:"0 0 40px" }}>
        ...
        <div style={{ width:"100%", background:"#1C3557", padding:"16px 20px", display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 12px rgba(28,53,87,0.3)" }}>
          <a href="/" style={{ display:"flex", alignItems:"center", gap:10, textDecoration:"none" }}>
            ...
            <div>
              <div style={{ fontSize:16, fontWeight:800, color:"#fff", fontFamily:"'DM Sans',sans-serif", letterSpacing:"-0.01em" }}>automarket<span style={{ color:"#E63027" }}>.de</span></div>
          ...
          {submitted ? (
            ...
            <>
              ...
              <div style={{ padding:"16px 20px", borderTop:"1px solid #F0F4F8", display:"flex", gap:10, position:"sticky", bottom:0, background:"#fff" }}>
                {step > 0 && (
                  <button onClick={() => setStep(s => s-1)} style={{ flex:1, padding:"14px", borderRadius:12, border:"1.5px solid #DDE3EA", background:"transparent", color:"#1C3557", fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>← Zurück</button>
                )}
                <button
                  ...
                  {submitting ? (
                    <><span style={{ width:16, height:16, border:"2px solid rgba(255,255,255,0.3)", borderTopColor:"#fff", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }} />Wird veröffentlicht…</>
        ...
        {!submitted && (
          <div style={{ display:"flex", gap:16, marginTop:20, padding:"0 20px" }}>
            {["🔒 SSL-verschlüsselt","🛡️ Datenschutz","✅ Kostenlos"].map(b => (
              <div key={b} style={{ fontSize:11, color:"#8A9BAE", fontFamily:"'DM Sans',sans-serif" }}>{b}</div>
            ))}
          </div>
        )}
      </div>
